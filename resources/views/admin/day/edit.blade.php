    ...
    <div class="col-12 bg-white p-0">
        ...
        <div class="block">
            ...
            <div class="block-content">
                <form action="{{ route('day.update', ['day' => $day->id]) }}" method="POST" enctype="multipart/form-data">
                    @method('PUT')
                    @csrf
                                ...
                                <div class="col-12">
                                    <select class="js-select2 form-control" id="meals"
                                        ...
                                        @foreach($meals as $meal)
                                            <?php
                                                $arr = [];
                                                foreach ($day->meals as $chosenMeal)
                                                {
                                                    $arr[] = $chosenMeal->id;
                                                }
                                            ?>
                                            <option value="{{ $meal->id }}" {{ in_array($meal->id, $arr) ? 'selected' : '' }}>{{ $meal->title }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
